...
class SettingsController extends Controller
{
    ...
    public function general()
    {
        ...
        return view('admin.settings.General.general', compact('settings', 'timezonelist', 'currencies', 'config'));
    }
    ...
    public function generalStore(Request $request)
    {
        ...
        try {
            ...
            $setting->edahab_api_key = $request->edahab_api_key;
            $setting->edahab_agent_code = $request->edahab_agent_code;
            $setting->zaad_merchant_uid = $request->zaad_merchant_uid;
            $setting->zaad_user_id = $request->zaad_user_id;
            $setting->zaad_api_key = $request->zaad_api_key;
            ...
            $setting->recaptcha_enable = $request->recapcha_status ? true : false;
            if (isset($request->recapcha_status)) {
                $setting->recapcha_site_key = $request->site_key;
            }
            ...
            $envData['APP_NAME'] = $request->timezone;
            $envData['APP_TYPE'] = $request->app_type;
            if ($request->cookie) {
                $envData['COOKIE_CONSENT_ENABLED'] = $request->cookie;
            }


            if ($request->google_auth_enable) {
                $envData['GOOGLE_ENABLE'] = $request->google_auth_enable;
            }

            if ($request->google_client_id) {
                $envData['GOOGLE_CLIENT_ID'] = $request->google_client_id;
            }
            if ($request->google_client_secret) {
                $envData['GOOGLE_CLIENT_SECRET'] = $request->google_client_secret;
            }
            if ($request->google_redirect) {
                $envData['GOOGLE_REDIRECT'] = $request->google_redirect;
            }

            $envData['SIZE_LIMIT'] = 1024;
            // dd($envData);
            ...
            dd($e->getMessage());
            DB::rollback();
            Toastr::error('Settings not Updated', 'Info');
...
//        ]);
        ...
        if ($request->has('mail_host')) {
            $envData['MAIL_HOST'] = $host;
        }
        if ($request->has('mail_port')) {
            $envData['MAIL_PORT'] = $port;
        }
        ...
        if ($request->has('mail_encryption')) {
            $envData['MAIL_ENCRYPTION'] = $encryption;
        }
        if ($request->has('mail_address')) {
            $envData['MAIL_FROM_ADDRESS'] = $from_address;
        }
        if ($request->has('mail_sender')) {
            $envData['MAIL_FROM_NAME'] = $from_name;
        }

        self::changeEnv($envData);
        self::clear();
    ...
    public function testEmail(Request $request)
    {
        ...
        $request->validate(['test_email' => 'required']);
        $message = [
            'msg' => 'Test mail'
        ];

        try {
            Mail::to($request->test_email)->send(new TestMail($message));
        } catch (Exception $e) {
    ...
    {
        $url = uploadImage($request->image, "editor", 780, 550);
