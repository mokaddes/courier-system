                                                    ...
                                                    <td>
                                                        @if (Auth::user()->can('admin.region.edit'))
                                                            <a href="javascript:void(0)"
                                                                ...
                                                                data-id="{{ $row->id }}"><i class="fa fa fa-edit" data-toggle="tooltip" data-placement="top" title="Edit"></i></a>
                                                        @endif

                                                        @if (Auth::user()->can('admin.region.delete'))
                                                            <a href="{{ route('admin.region.delete', $row->id) }}"
                                                                id="deleteData" class="btn btn-danger btn-sm"><i class="fa fa-trash-alt" data-toggle="tooltip" data-placement="top" title="Delete"></i></a>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                            ...
                            <select name="country_id" id="country_id" class="form-control">
                                @if (isset($data['country']) && count($data['country']) > 0)
                                    @foreach ($data['country'] as $key => $item)
                                        <option @if($item->id == old('country_id')) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
...
@push('script')
    <script type="text/javascript">
        $(document).on('click', '.edit', function() {
            ...
            $.get('region/' + cat_id + '/edit', function(data) {
